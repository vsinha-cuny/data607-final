...
First we consolidate all data into one data.frame.
...
for (cty in Ccodes2) {
    ...
    cty3 = Ccodes3[i]
    i = i + 1
    ll = PatentsByCountry[unlist(cty2)]
    df = data.frame(ll)
    names(df) = c("Year", "Applications")
    df2 = spread(df, Year, Applications)
    names(df2) = paste("P", seq(from=2007, to=2016), sep="")
    df2 = df2 %>% select(c(P2010:P2016))
    df2$Country.Code = as.factor(unlist(cty3))
    # Combine with expenditures into 1 dataframe
    patents = rbind(patents, df2)
}

xdf = inner_join(Expenditures, patents)

# Next, drop columns for which we have incomplete data
xdf = xdf %>% select(-c(X2010, X2011, X2015, X2016, P2010, P2011, P2015, P2016))

# Divide patents counts by population means to get per-capita patents.

xdf$P2012 = xdf$P2012/Populations$Mean
xdf$P2013 = xdf$P2013/Populations$Mean
xdf$P2014 = xdf$P2014/Populations$Mean
